...
Роль в Big Data | Базовое количественное «сканирование» массивов данных для их обзора, контроля качества и подготовки к дальнейшему моделированию и принятию решений.

Роль в Big Data:
- Позволяет быстро суммировать и описать огромные массивы данных с помощью агрегатов и простых визуализаций (средние, медианы, процентили, гистограммы, боксплоты).

- Даёт первичное понимание распределений, масштабов, аномалий и пробелов в данных, что критично при работе с высокоразмерными и «грязными» Big Data.

Место в жизненном цикле данных:
- Используется на этапе Data Exploration / Exploratory Data Analysis как фундаментальный шаг перед любыми статистическими тестами и ML‑моделями.

- Помогает сформировать или уточнить гипотезы, определить нужные преобразования признаков и выбрать адекватные методы анализа для больших данных.

Практические задачи в Big Data:
- Оценка качества данных: поиск пропусков, выбросов, несоответствий, ошибок выгрузки и интеграции из разных источников.

- Снижение размерности на уровне практики: отбор информативных признаков, первичная агрегация и группировка, чтобы сделать последующий анализ масштабируемым.
...
| Роль в Big Data | Формальное подтверждение (или опровержение) различий и эффектов в больших выборках, контроль ошибок и достоверности выводов, особенно при множественных тестах и A/B‑эксперимента

Роль в Big Data:
- Позволяет проверять, являются ли обнаруженные на этапе EDA закономерности в больших массивах данных статистически надежными, а не результатом случайных флуктуаций или шума.

- Даёт формализованный критерий для принятия решений (запуск/остановка продукта, выбор модели, изменение бизнес‑процесса) на основе p‑value, доверительных интервалов и размеров эффекта.

Особенности при больших данных:
- В больших выборках даже очень малые эффекты становятся статистически значимыми, поэтому возрастает роль оценки размера эффекта и практической значимости, а не только p‑value.

- Требует учёта множественных проверок гипотез (коррекции p‑значений, контроль FDR по Беньямини–Хохбергу и др.), так как тесты часто запускаются по сотням и тысячам признаков одновременно.

Практические задачи в Big Data:
- Используется для анализа результатов A/B‑тестов, онлайн‑экспериментов, клинических и промышленных испытаний, когда данные собираются в больших объёмах и непрерывно.

- Применяется для оценки значимости признаков и проверки предпосылок моделей (нормальность, гомоскедастичность и др.), а также в бутстрэп‑ и перестановочных тестах, которые особенно удобны при работе с большими данными
...
Роль в Big Data | Выявление и количественная оценка связей между признаками в больших массивах данных, упрощение моделей за счёт удаления избыточных признаков и диагностика мультиколлинеарности.

Роль в Big Data:
- Позволяет строить корреляционные матрицы для сотен и тысяч признаков, чтобы увидеть структуру взаимосвязей и кластеры тесно связанных переменных.

- Даёт основу для принятия решений об исключении или агрегировании признаков, снижая размерность и сложность моделей, что особенно важно при больших объёмах данных.

Отбор признаков и мультиколлинеарность
- Используется как простой и масштабируемый инструмент отбора признаков (*feature selection*): сильно коррелирующие признаки (например, с $r>0,7$) часто исключают или заменяют более компактными представлениями (PCA и др.).

- Является базовым методом диагностики мультиколлинеарности в регрессионных и других моделях: высокая взаимная корреляция объясняющих переменных приводит к нестабильности оценок и переобучению, поэтому избыточные факторы удаляют.

Практические задачи в Big Data:
- Применяется для анализа корреляций между бизнес‑метриками, техническими метриками и пользовательским поведением, чтобы находить информативные признаки и потенциальные конфаундеры.

- Используется перед обучением ML‑моделей для построения более интерпретируемых и устойчивых моделей, ускорения обучения и уменьшения требований к ресурсам за счёт сокращения числа признаков.
...
Роль в Big Data | Выявление значимых категориальных факторов и их взаимодействий, формальная проверка влияния различных условий/групп на числовые метрики и поддержка отбора признаков для моделей.

Роль в Big Data:
- Позволяет оценивать, какие категории (группы пользователей, виды продуктов, экспериментальные условия) приводят к статистически значимым различиям целевой метрики в больших выборках.

- Помогает численно измерять вклад факторов и их комбинаций в вариацию показателя, что важно при анализе сложных систем с множеством категориальных признаков.

Отбор категориальных признаков:
- Используется как метод отбора признаков (feature selection) для категориальных факторов: значимые по F‑критерию и p‑значению факторы включают в модель, а незначимые можно исключать или агрегировать.

- В многофакторном ANOVA позволяет выявлять не только основной эффект каждого фактора, но и эффекты взаимодействия (interaction), что помогает находить сочетания категорий, существенно влияющие на метрики.

Практические задачи в Big Data:
- Применяется для сравнения эффективности нескольких вариантов продукта, рекламных кампаний, условий эксплуатации и т.п., когда данных много и группы многочисленны.

- Используется в автоматизированной аналитике и BI‑системах для оценки «прогностической силы» категориальных факторов по отношению к целевой метрике, поддерживая построение интерпретируемых моделей и дашбордов.
...
| Роль в Big Data | Построение интерпретируемых предиктивных моделей, оценка влияния множества признаков на целевую переменную и служба базовой точкой отсчёта для сравнения более сложных алгоритмов.

Роль в Big Data:
- Позволяет формально описывать и количественно оценивать влияние множества факторов на целевую метрику (доход, риск, спрос, отклик пользователя) на больших массивах данных.

- Даёт базовую предиктивную модель (*baseline*), с которой сравнивают сложные методы (ансамбли, нейросети), и часто остаётся предпочтительным решением, когда важны простота и интерпретируемость.

Регрессия как инструмент МО:
- Линейная и регуляризованные регрессии (Ridge/Lasso/ElasticNet) — ключевые алгоритмы обучения с учителем для задач непрерывного прогноза, хорошо масштабируемые к большим выборкам и числу признаков.

- Коэффициенты регрессии используются для оценки важности признаков и направления влияния, а метрики (R², MSE, RMSE, MAE) — для выбора и сравнения моделей на больших данных.

Практические задачи в Big Data:
- Применяется для прогнозирования цен, спроса, вовлечённости пользователей, загрузки ресурсов и других бизнес‑метрик, когда есть исторические данные в больших объёмах.

- Используется в качестве интерпретируемого инструмента для объяснения решений моделей (explainable AI), проверок гипотез о влиянии признаков и построения прозрачных скоринговых и риск‑моделей.
...
| Роль в Big Data | Моделирование времени до события и риска его наступления с учётом цензурированных наблюдений, оценка влияния множества факторов и построение риск‑моделей для медицины, финансов и клиентской аналитики.

Роль в Big Data:
- Позволяет оценивать вероятность наступления события (смерть, дефолт, отток клиента, отказ оборудования) во времени и сравнивать риск между группами на больших массивах данных.

- Даёт инструменты для работы с цензурированными данными, которые типичны для реальных больших выборок (часть объектов ещё «жива» к моменту анализа).

В машинном обучении и рисках:
- Модели выживаемости (Каплан–Майер, регрессия Кокса, деревья/ансамбли выживаемости) используются для прогнозирования функции выживания и функции риска как специализированный вид задач с учителем.

- Активно применяются в медицинском ML, кредитном скоринге, прогнозе оттока клиентов и оценке LTV, где важно не только «наступит ли событие», но и «когда оно с высокой вероятностью наступит».

Практические задачи в Big Data:
- Используются для построения долгосрочных риск‑моделей на потоках данных (электронные медкарты, телеметрия устройств, транзакции клиентов), учитывая динамику риска во времени.

- Служат основой для интерпретируемых моделей в рамках explainable AI: исследуются, какие признаки влияют на форму функции выживания и hazard, и как их изменение изменяет риск и время до события.
...
| Роль в Big Data | Выявление скрытых структур и групп признаков, упрощение высокоразмерных данных и создание компактных, интерпретируемых представлений для последующего моделирования.

Роль в Big Data:
- Позволяет сократить размерность датасетов с десятками и сотнями коррелирующих признаков, заменяя их меньшим числом латентных факторов при сохранении основной информации.

- Выявляет скрытые структуры (темы, компоненты удовлетворённости, типы поведения), которые трудно увидеть напрямую в сыром высокоразмерном пространстве признаков.

В машинном обучении и предобработке:
- Используется как метод уменьшения размерности и построения новых признаков: факторные оценки (*factor scores*) добавляют в модели вместо исходных переменных, снижая шум и мультиколлинеарность.

- Помогает сделать модели более интерпретируемыми: вместо десятков частных вопросов или метрик используются несколько факторов («качество», «сервис», «цена» и т.п.), по которым легче анализировать влияние.

Практические задачи в Big Data:
- Применяется в опросах, маркетинговой и поведенческой аналитике, рекомендательных системах и сегментации клиентов, где много взаимосвязанных признаков и нужны компактные описания.

- Используется как шаг предобработки перед регрессией, кластеризацией и другими моделями, чтобы снизить размерность, улучшить устойчивость оценок и ускорить обучение на больших выборках
...
| Роль в Big Data | Сжатие высокоразмерных данных в небольшое число главных компонент, устранение мультиколлинеарности и ускорение/стабилизация последующего обучения моделей при минимальной потере информации

Роль в Big Data
- Позволяет уменьшать размерность датасетов с десятками и сотнями признаков, сохраняя основную часть дисперсии и делая данные более компактными и удобными для анализа.

- Преобразует коррелирующие переменные в набор некоррелированных главных компонент, что упрощает структуру данных и снижает влияние мультиколлинеарности на модели.

В машинном обучении и предобработке:
- Является одним из ключевых алгоритмов обучения без учителя для уменьшения размерности и построения новых признаков, на которых далее обучают регрессию, классификацию, кластеризацию и другие модели.

- Используется для визуализации многомерных данных в 2D/3D, поиска структур и кластеров, а также для декоррелирования признаков, что повышает устойчивость и скорость обучения на больших выборках.

Практические задачи в Big Data:
- Применяется в обработке табличных, текстовых, геномных и других высокоразмерных данных, чтобы уменьшить количество признаков перед сложными моделями (ансамбли, нейросети) и снизить требования к ресурсам.

- Используется в системах анализа и визуализации данных (BI, аналитические платформы) для построения проекций данных, выявления аномалий и структур, которые трудно заметить в исходном пространстве признаков.
...
| Роль в Big Data | Классическая, интерпретируемая модель многоклассовой классификации и контролируемого уменьшения размерности, которая хорошо работает как базовый алгоритм и даёт прозрачные правила разделения классов
...
Роль в Big Data:
- Позволяет строить простые, аналитически задаваемые классификаторы (LDA/QDA), которые разделяют известные группы по линейным или квадратичным границам и часто используются как первая «рабочая» модель.

- Выявляет признаки (или их линейные комбинации), которые лучше всего различают классы, что делает модель не только предсказательной, но и хорошо интерпретируемой для экспертов и бизнеса.

В машинном обучении и уменьшении размерности:
- LDA и QDA — классические алгоритмы обучения с учителем: они моделируют распределения классов и выводят решающие правила, не требуя сложной настройки гиперпараметров.

- Линейный дискриминантный анализ используется как метод контролируемого понижения размерности (до $C−1$ компонент при $C$ классах), сохраняя максимально дискриминативную информацию о разделении классов.

Практические задачи в Big Data:
- Применяется в медицинской диагностике, кредитном скоринге, распознавании образов и других задачах классификации, где важны и качество разделения, и интерпретируемость границ между классами.

- Используется как базовый ориентир для сравнения более сложных моделей (логистическая регрессия, ансамбли) и как инструмент построения низкоразмерных признаков для последующего обучения сложных классификаторов.
...
| Роль в Big Data | Автоматическое выявление скрытых групп и структур в данных без меток, сегментация объектов по похожим паттернам поведения или свойствам и поддержка задач сегментации и обнаружения аномалий
...
Роль в Big Data:
- Позволяет разбивать большие массивы объектов на однородные кластеры, где элементы внутри группы максимально похожи, а между группами — существенно различаются.

- Даёт способ «дать данным заговорить самим за себя» — находить естественные сегменты и паттерны без заранее заданных классов, что особенно важно при исследовании новых или слабо структурированных данных.

В машинном обучении и сегментации:
- Является ключевым классом алгоритмов обучения без учителя (K-means, иерархическая кластеризация, DBSCAN, GMM и др.), используемых для сегментации клиентов, документов, товаров, сессий и других объектов.

- Служит основой для обнаружения аномалий: объекты, плохо вписывающиеся ни в один кластер, рассматриваются как потенциально аномальные или требующие отдельного анализа.

Практические задачи в Big Data:
- Применяется для построения маркетинговых сегментов, выделения групп пользователей по паттернам поведения, кластеризации текстов по темам и разделения сложных многомерных данных на интерпретируемые группы.

- Используется как шаг предобработки: кластерные метки или расстояния до центров кластеров добавляют как новые признаки в модели, улучшая их качество и интерпретируемость в больших системах аналитики.
...
| Роль в Big Data | Упрощение признаков и моделей за счёт преобразования многочисленных или непрерывных значений в ограниченное число интерпретируемых категорий, снижение переобучения и повышение стабильности и понятности решений.

Роль в Big Data:
- Позволяет дискретизировать непрерывные и многозначные категориальные признаки, уменьшая число уникальных значений и делая данные более структурированными и удобными для анализа и моделирования.

- Обеспечивает создание осмысленных интервалов («низкий/средний/высокий доход», возрастные группы, рейтинговые классы), что облегчает интерпретацию результатов и коммуникацию с бизнесом и экспертами.

В машинном обучении и предобработке:
- Является важной техникой предобработки и feature engineering: дискретизированные признаки часто лучше подходят для деревьев решений, наивного Байеса и правил‑ориентированных моделей, улучшая их стабильность и защищённость от шума и выбросов.

- Помогает бороться с переобучением, сглаживая мелкие флуктуации и измерительный шум, а также позволяет кодировать пороговые и регуляторные эффекты (например, возраст/доход выше/ниже определённого порога).

Практические задачи в Big Data:
- Применяется для построения рейтинговых шкал, риск‑градаций, клиентских сегментов по диапазонам значений (возраст, доход, активность), а также для подготовки признаков в скоринговых и отчётных моделях.

- Используется в больших ML‑системах для стандартизованного кодирования признаков: единая логика биннинга позволяет поддерживать устойчивость моделей во времени и облегчает контроль и аудит решений.
...
| Роль в Big Data | Основной инструмент моделирования динамики и прогнозирования показателей во времени, учитывая тренды, сезонность и зависимость от прошлых значений.

Роль в Big Data:
- Позволяет описывать структуру временных процессов (тренд, сезонность, шум, циклы) и строить прогнозы на основе исторических данных, что критично для управления бизнес‑метриками и ресурсами.

- Даёт формальные модели (ARIMA, SARIMA, GARCH, VAR, ML‑модели для рядов), которые используют автокорреляции и временную зависимость для получения более точных и устойчивых прогнозов.

В машинном обучении и прогнозировании:
- Временные ряды лежат в основе задач предсказания (*forecasting*): от классических статистических моделей до нейросетевых архитектур (RNN, LSTM, ансамбли), адаптированных к последовательным данным.

- Особые схемы разбиения на train/test и кросс‑валидации (TimeSeriesSplit, «скользящее окно») используются для корректной оценки качества моделей, учитывая порядок наблюдений и утечку информации.

Практические задачи в Big Data:
- Применяется для прогноза спроса, цен, трафика, загрузки инфраструктуры, погодных и медицинских показателей, а также для обнаружения аномалий и изменений режима в потоковых данных.

- Используется в финансовой аналитике, логистике, телеком‑сфере и здравоохранении как ключевой инструмент для планирования, управления рисками и мониторинга качества процессов на больших временных выборках
