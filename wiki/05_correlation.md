...
- [05. Корреляционный анализ](#05-корреляционный-анализ)
  ...
  - [Парные статистические связи](#парные-статистические-связи)
    ...
    - [Параметрическая корреляция](#параметрическая-корреляция)
    ...
    - [Непараметрическая (ранговая) корреляция](#непараметрическая-ранговая-корреляция)
      - [Коэффициент ранговой корреляции Спирмена (неметрические шкалы)](#коэффициент-ранговой-корреляции-спирмена-неметрические-шкалы)
        ...
        - [Присвоение средних рангов](#присвоение-средних-рангов)
      ...
      - [Хи-квадрат статистика](#хи-квадрат-статистика)
      - [Таблицы сопряженности](#таблицы-сопряженности)
      - [Меры ассоциации](#меры-ассоциации)
  ...
  - [Практическая работа. Исследование парных связей между количественными и бинарными признаками](#практическая-работа-исследование-парных-связей-между-количественными-и-бинарными-признаками)
    - [Задачи](#задачи-1)
    - [Пример расчета](#пример-расчета-2)
...
![Линия регресси](../img/regression_line.png)
...
2. **Коэффициент корреляции Кендалла** ($\tau$)
    ...
    $$ \tau = \frac{P - Q}{\sqrt{(P + Q + T_x)(P + Q + T_y)}} $$

    где $P$ — число конкордантных пар, $Q$ — число дискордантных пар, $T_x$ и $T_y$ — поправки на связанные ранги.
...
3. **Ранговая корреляция Спирмена** ($\rho$)
    ...
    где $d_i$ — разности между соответствующими рангами, $n$ — число наблюдений.
...
4. **Коэффициент корреляции Гудмена-Краскела** ($\gamma$)
    ...
    где $P$ — число конкордантных пар, $Q$ — число дискордантных пар.
...
5. **Коэффициент фи-корреляции для бинарных переменных** ($\phi$)
    ...
    где $a, b, c, d$ — частоты совместного появления категорий двух бинарных переменных.
...
7. **Коэффициент Крамера для номинальных переменных**
    ...
    где $χ^2$ — значение критерия хи-квадрат, $n$ — общее число наблюдений, $r$ и $c$ — число строк и столбцов таблицы сопряжённости.
...
8. **Бисериальный коэффициент корреляции**
    ...
    $$ r_{bis} = \frac{(x_1 - x_0) \sqrt{p q}}{\sigma_y} $$

    где $x_1$ — среднее значение $y$ для группы с кодом 1, $x_0$ — среднее для группы с кодом 0, $p$ — доля единиц, $q=1−p$, $σ_y$ — стандартное отклонение $y$.
...
9. **Рангово-бисериальный коэффициент корреляции**
    ...
    $$ r_{rb} = \frac{2(n_1 n_0)}{n} \cdot \frac{\bar{R}_1 - \bar{R}_0}{n - 1} $$

    где $n_1, n_0$ — размеры групп с кодами 1 и 0, $\bar{R_1}, \bar{R_0}$ — средние ранги этих групп, $n$ — общее число наблюдений.
...
- <dfn title="нулевая гипотеза">Нулевая гипотеза</dfn> (обозначается $H_0$) – утверждение о параметре генеральной совокупности (параметрах генеральных совокупностей) или распределении, которое необходимо проверить. Это предположение о том, что между изучаемыми параметрами нет значимых различий или связи, то есть наблюдаемые различия являются случайными. Проще говоря, нулевая гипотеза – это утверждение об отсутствии эффекта или различий (например, «новый метод лечения не эффективнее старого»).
    ...
    Нулевая гипотеза ($H_0$): постулирует отсутствие различий или эффекта. Это статус-кво, которое мы пытаемся опровергнуть. Пример: "Средний рост мужчин и женщин в данном городе одинаков." Например, $H_0 : μ_1 = μ_2$ утверждает, что средние значения двух групп равны.
...
Нулевая гипотеза всегда проверяется на определенном уровне значимости. <dfn title="уровень значимости">Уровень значимости</dfn> (обозначается как $α$) – вероятность отвергнуть верную нулевую гипотезу. Например, если мы проверяем нулевую гипотезу на уровне значимости 5%, это означает, что если мы будем проводить аналогичные исследования 100 раз и проверять на основе имеющихся данных интересующую нас нулевую гипотезу, в 5 случаях из 100 мы отвергнем нулевую гипотезу, хотя она будет верной.
...
**Аналогия**: суд над предступником $H_0$: заключенный не виновен.
...
- Таким образом, $p$-значение является инструментом, который помогает критерию принять решение о статистической значимости результатов, а критерий задаёт сам процесс проверки гипотезы.
...
#### Выводы
Проверка статистических гипотез и оценка статистической значимости составляют основу индуктивной статистики, позволяя на базе данных выборки делать обоснованные выводы о параметрах генеральной совокупности, минимизируя влияние случайности. Процесс включает формулировку нулевой гипотезы $H_0$ (отсутствие эффекта) и альтернативной $H_1$, выбор уровня значимости $α$ (обычно 0,05), расчёт тестовой статистики (t-критерий, $χ^2$ и др.) и p-значения, которое показывает вероятность наблюдаемых данных при верной $H_0$; если $p < α$, $H_0$ отвергается в пользу $H_1$. Этот метод критически важен в науке и практике для проверки различий, но требует учёта мощности теста, размера эффекта и рисков ошибок I и II рода, чтобы выводы были не только статистически значимыми, но и практически полезными.
...
- **Номинальная** (**шкала наименований**): группирует объекты по категориям, не предполагая никакой упорядоченности. Пример: виды транспорта, национальность, пол.
- **Порядковая**: классифицирует объекты в соответствии с принципом "больше – меньше", но не указывает, насколько велика разница между ними. Пример: результаты опроса "не согласен", "нейтрально", "согласен" или уровень образования.
...
<dfn title="параметрическая корреляция">Параметрическая корреляция</dfn> — это статистический метод оценки степени и направления линейной связи между двумя количественными переменными, которые предполагаются нормально распределёнными. Наиболее распространённым примером является коэффициент корреляции Пирсона, который измеряет, насколько изменения в одной переменной связаны с изменениями в другой в линейной форме.

Основные особенности параметрической корреляции:

- предполагает нормальное распределение данных и линейную связь между переменными;

- коэффициент корреляции Пирсона принимает значения от -1 до +1, где +1 означает идеальную прямую зависимость, -1 — идеальную обратную, а 0 — отсутствие линейной связи;

- используется для измерения степени тесноты линейной связи и направления этой связи (положительной или отрицательной);

- метод расчёта включает вычисление ковариации между переменными, нормированной на произведение их стандартных отклонений.

Параметрическая корреляция широко применяется в статистике, экономике, медицине, социальных науках и других областях для анализа взаимосвязей между количественными признаками с учётом предпосылки о нормальности данных.
...
Коэффициент корреляции Пирсона — это классический инструмент для оценки силы линейной связи между переменными.
...
- Требует нормального распределения обеих переменных X и Y.
- Переменные должны измеряться в интервальной или отношении шкале (метрические данные).
- Объём выборки для обеих переменных должен быть одинаковым.
- Коэффициент корреляции Пирсона очень чувствителен к **выбросам** (*outliers*): даже один аутлаер может значительно исказить результат.
- Не выявляет нелинейные зависимости, даже если они сильны.
...
##### Выводы
Парные статистические связи описывают взаимосвязь между двумя переменными, измеряемыми в метрических шкалах (интервальной или отношений), где ковариация показывает, изменяются ли они в одном направлении (положительная) или противоположных (отрицательная), но не нормализует силу связи. Параметрическая корреляция, такая как коэффициент корреляции Пирсона (r), решает эту проблему, оценивая только линейную связь в диапазоне от -1 до +1 при нормальном распределении данных, требуя одинаковых выборок и чувствительности к аутлаерам. Этот метод идеален для метрических данных без выбросов, но не выявляет нелинейности и не доказывает причинности, подчеркивая необходимость проверки предположений перед применением.
...
###### Алгоритм расчета
Если у имеется одна группа с двумя признаками — один количественный, другой бинарный (дихотомический), то для вычисления бисериального коэффициента корреляции используется следующая схема:
...
2. Вычислить средние значения количественного признака для каждой из этих групп: $\bar{Y_1}$ и $bar{Y_0}$.
...
#### Непараметрическая (ранговая) корреляция
<dfn title="непараметрическая корреляция">Непараметрическая корреляция</dfn> — это метод измерения степени связи между двумя переменными без предположения о конкретном распределении данных (например, без требования нормальности). Такие методы основаны на ранжировании данных и подходят для количественных и порядковых признаков, а также для случаев с выбросами или ненормальным распределением.

Основные характеристики непараметрической корреляции:

- не требует нормального распределения признаков и линейной зависимости;

- часто используется коэффициент ранговой корреляции Спирмена, который вычисляется по рангам значений и отражает монотонную связь между переменными;

- коэффициент Кенделла — ещё один популярный непараметрический метод, оценивающий согласованность парных сравнений рангов;

- устойчивы к выбросам и дают более надёжную оценку взаимосвязи при сложных распределениях или небольших выборках.

Непараметрическая корреляция применяется в случаях, когда данные не соответствуют требованиям параметрических методов, и необходим изучить обобщённые закономерности взаимосвязей, включая нелинейные и монотонные зависимости.

##### Коэффициент ранговой корреляции Спирмена (неметрические шкалы)
<dfn title="коэффициент ранговой корреляции">Коэффициенты ранговой корреляции</dfn> используются для измерения взаимозависимости между качественными признаками, значения которых могут быть упорядочены или проранжированы по степени убывания (или возрастания) данного качества у исследуемых социальных объектов.
...
Каждому значению в обоих признаках присваивается <dfn title="ранг">ранг</dfn> — порядковый номер после упорядочивания по возрастанию (либо убыванию). Затем уже вычисляются разности рангов соответствующих наблюдений для подсчёта итогового коэффициента.
...
###### Присвоение средних рангов
Присвоение средних рангов применяется, когда в данных встречаются одинаковые значения (так называемые «связки»). Вместо того, чтобы присваивать одинаковым значениям разные последовательные ранги, им присваивают среднее арифметическое тех рангов, которые они получили бы, если бы значения были все уникальными.

Например, если в упорядоченном списке имеются три одинаковых значения, которые могли бы занимать ранги 5, 6 и 7, то каждому из этих одинаковых значений присваивается средний ранг:

$$ \frac{5+6+7}{3} = 3 $$

То есть все три равных значения получают ранг 6.

Это позволяет сохранить сумму всех рангов и корректно учитывать равенства при вычислениях, например при расчете коэффициента ранговой корреляции Спирмена. Без усреднения рангов такие повторяющиеся значения могли бы дать неверное представление о разнице между наблюдениями.

Таким образом, средние ранги обеспечивают корректное обработку связных данных и сохраняют статистическую корректность ранжирования в анализе.
...
| Посещаемость | Ранг посещаемости | Оценка | Уникальный ранг | Ранг оценки |
| ------------ | ----------------- | ------ | --- | ----------- |
| 95           | 1                 | 5      | 1 | 1.5 = (1+2)/2        |
| 90           | 2                 | 5      | 2 | 1.5         |
| 85           | 3                 | 4      | 3 | 4.5 = (3+4+5+6)/4         |
| 80           | 4                 | 4      | 4 | 4.5         |
| 75           | 5                 | 4      | 5 | 4.5         |
| 70           | 6                 | 4      | 6 | 4.5         |
| 60           | 7                 | 3      | 7 | 8.0 = (7+8+9)/3         |
| 50           | 8                 | 3      | 8 | 8.0         |
| 40           | 9                 | 3      | 9 | 8.0         |
| 30           | 10                | 0      | 10 | 10.0          |
| **∑** | **55** |  | **55** | **55**

$$ \frac{n(n+1)}{2} = \frac{10 \cdot 11}{2} = \frac{110}{2} = 55 $$
...
4. Подстановка в формулу коэффициента Спирмена:
    ...
    $$ \rho = 1 - \frac{6 \sum d_i^2}{n (n^2-1)} = 1 - \frac{6 \cdot 7.5}{10 (10^2-1)} = 1 - \frac{45}{990} = 1-0.0(45) = 0.95(45) $$

    где $d_i$ — разница рангов по двум признакам, $n=10$.

    Коэффициент ранговой корреляции Спирмена равен примерно 0.95, что указывает на очень высокую положительную монотонную связь между посещаемостью лекций и оценкой студентов. Чем выше посещаемость, тем выше аттестационная оценка в общем.
...
5. Оценить статистическую значимость через критерий $t$ с $n−2=8$ степенями свободы.
    ...
    Для оценки статистической значимости полученного коэффициента ранговой корреляции Спирмена $r_s = 0.95$ с размером выборки $n=10$ используется t-критерий Стьюдента с формулой:

    $$ t = r \sqrt{\frac{n-2}{1-r^2}} $$

    Подставим значения:

    $$ t = 0.95 \sqrt{\frac{10-2}{1-0.95^2}} \approx 0.95 \cdot 9.06 \approx 8.61 $$

    Число степеней свободы равно $n−2=8$.

    Далее нужно сравнить вычисленное значение $t=8.61$ с критическим значением $t$ для 8 степеней свободы при уровне значимости $α=0.05$, которое примерно равно 2.306.

    Так как 8.61 значительно больше 2.306, нулевая гипотеза о том, что корреляция отсутствует, отвергается. Следовательно, полученный коэффициент ранговой корреляции Спирмена статистически значим.
...
- Значение коэффициента Спирмена $ρ≈0.95$ — сильная положительная корреляция.
...
<dfn title="хи-квадрат статистика">Хи-квадрат статистика</dfn> ($χ²$) — это непараметрический статистический критерий, используемый для проверки гипотез о соответствии наблюдаемых данных ожидаемым распределениям или о независимости категориальных переменных.

Критерий хи-квадрат, разработанный Карлом Пирсоном, измеряет расхождение между наблюдаемыми частотами в выборке и ожидаемыми значениями под нулевой гипотезой (например, о нормальном распределении или независимости). Его значение рассчитывается по формуле:

$$ χ^2 = \sum{\frac{(O_i - E_i)²}{E_i}} $$

где $O_i$ — наблюдаемые частоты, $E_i$ — ожидаемые. Распределение статистики приближается к $χ²$ при больших выборках, что позволяет сравнивать с табличными критическими значениями.

В контексте мер ассоциации хи-квадрат используется для анализа таблиц сопряжённости, выявляя статистически значимую связь между номинальными переменными.

##### Таблицы сопряженности
<dfn title="таблица сопряженности">Таблица сопряженности</dfn> (*contingency table*) — средство представления совместного распределения двух переменных, предназначенное для исследования связи между ними. Таблица сопряженности является наиболее универсальным средством изучения статистических связей, так как в ней могут быть представлены переменные с любым уровнем измерения. Таблицы сопряженности используются для проверки гипотезы о наличии связи между двумя признаками, а также для измерения тесноты связи.[^statistika-tabliczy-sopryazhennosti]

<dfn title="таблицы сопряжённости">Таблицы сопряжённости</dfn> — это способ представления совместного распределения двух категориальных переменных в виде двумерной таблицы для анализа их статистической связи. Строки таблицы соответствуют значениям одной переменной, столбцы — другой (количественные данные предварительно группируются в интервалы), а в ячейках указываются частоты совместного появления значений. Они универсальны для номинальных и порядковых шкал, позволяя выявлять зависимости между признаками.

Таблицы используются с критерием хи-квадрат для проверки гипотезы о независимости переменных или с точным тестом Фишера при малых выборках. В анализе ассоциаций они служат основой для расчёта мер связи, таких как коэффициенты Гудмана-Краскала.

##### Меры ассоциации
<dfn title="меры ассоциации">Меры ассоциации</dfn> — это статистические коэффициенты, показывающие силу и направление связи между категориальными переменными на основе таблиц сопряжённости.

Они оценивают отклонение от независимости переменных, где значение 0 указывает на отсутствие связи, а 1 (или -1) — на полную зависимость; примеры включают коэффициент V Краскала или φ для таблиц 2×2. В отличие от хи-квадрат, которая только проверяет значимость, меры ассоциации дают величину связи для сравнения.

В контексте PRE-мер Гудмана и Краскала (*lambda*, *tau*, *uncertainty*) они измеряют пропорциональное снижение ошибки предсказания одной переменной по другой, дополняя анализ таблиц сопряжённости. Подходят для номинальных данных, где ранговые коэффициенты неприменимы из-за отсутствия порядка.

##### Коэффициент ранговой корреляции Гудмена-Краскела
*[PRE]: Proportional Reduction in Error

Альтернативой мерам ассоциации, построенным на ХИ-квадрат статистике, являются коэффициенты, основанные на идее Л. Гудмана и Е. Краскала о «пропорциональной редукции ошибок» (Proportional Reduction in Error Measures — PRE). PRE-меры основаны на пропорциональном снижении ошибки предсказания и часто дают более интерпретируемые результаты для силы связи. Для надёжности требуется минимум 50 наблюдений и отсутствие малых ожидаемых частот.
...
<dfn title="коэффициент ранговой корреляции Гудмена-Краскела">Коэффициент ранговой корреляции Гудмена-Краскела</dfn> (*Goodman-Kruskal's gamma*, $γ$) — это мера ассоциации между двумя порядковыми признаками, основанная на сравнении количества конкордантных (согласованных) и дискородантных (несогласованных) пар наблюдений.
...
#### Вывод
Непараметрическая корреляция представляет собой класс статистических методов для оценки силы и направления связи между переменными без предположений о нормальности распределения или линейности зависимости.

Ключевыми являются коэффициенты Спирмена (ρ) и Кендалла (τ), основанные на рангах данных, которые выявляют монотонные связи в порядковых и количественных переменных. Для номинальных данных применяются таблицы сопряжённости с мерами ассоциации (хи-квадрат, V Краскала, PRE-меры Гудмана-Краскала), измеряющие отклонение от независимости.

Эти методы устойчивы к выбросам, малым выборкам и нелинейным зависимостям, в отличие от параметрического коэффициента Пирсона, требующего нормальности. Они дополняют друг друга: ранговые — для упорядоченных данных, ассоциативные — для категориальных.

Непараметрическая корреляция универсальна в реальных данных социальных наук, психологии и медицины, обеспечивая робастный анализ связей при слабых предположениях о распределении.
...
4. Рассчитываются пропорции групп:
...
5. Вычисляется бисериальный коэффициент корреляции:
...
6. Проверяется статистическая значимость выявленной корреляции.
   ...
   - Вычисляется t-статистика:
    ...
    $$
    t = r_{bis} \sqrt{\frac{df}{1-r_{bis}^2}} =
    0.778 \sqrt{\frac{8}{1-0.778^2}} \approx 3.50
    $$

    - Определяется критическое значение для степени свободы $df = 8$ и уровня значимости $α = 0.05$ (двусторонний тест):

    $$ t_{0.025, 8} = 2.3636 $$

    - Расчетная t-статистика сравнивается с пороговым значением:
...
Правильные ответы: июнь и июль, потому что именно в эти месяцы примерно у половины студентов прошёл день рождения (прибавился возраст), а у второй половины — ещё нет.

Нам надо понять в каком месяце КК будет ближе всего к 0, т.е. когда у нас будет наименьшая корреляция.

В течение года человеку может быть полных лет x до дня рождения и x+1 после дня рождения.

В начале года(1-2 месяц) мало кто еще отметил свое день рождения, в конце года(11-12) свое день рождения отметили уже наверное многие. Но мы точно можем сказать, что чем ближе к концу года, тем больше вероятность, что человек уже отметил свое день рождение. Вот она корреляция, поэтому когда мы находимся в середине года, мы не можем сказать, что этот человек вероятно уже отметил свое др или еще нет.

Почему, если мы говорим о середине года, июль и август не подходят? В году 12 месяцев, это число четное, значит серединой нельзя назвать одно число. В данном случае это будут 6 и 7 месяцы (июнь и июль). Июль и август — 7 и 8 соответственно.

В январе 2021 года большинству людей еще 20 лет, т.е. зависимость возраста и года рождения довольно таки явная. Тоже самое касается и декабря 2021 (многим уже 21 год), опять же зависимость возраста и года рождения хорошо прослеживается. А вот в середине года некоторым 20, а некоторым 21 (расположи месяцы с января по декабрь, вычеркивай слева и справа по одному месяцу, найдешь ответ).

Подумайте, в какой месяц для одного и того же года рождения было бы максимальное количество разных ответов.

Я бы переформулировал вопрос так: в какой месяц (какие два месяца) будет примерно равным соотношение между студентами, у кого уже прошел день рождения, и теми, у кого он еще будет, в текущем году. Соответственно в эти месяцы нельзя сказать, у какой группы студентов будет больший перевес — с прошедшими или предстоящими днями рождения, а значит корреляция будет близка к нулю в отличие от месяцев, где возникает перевес у одной из двух групп.

Предложу такую логику. Пусть у нас есть несколько студентов и все они для простоты 2000 года рождения. Мы в 2022 проводим опрос, как в задании. В 2022 на момент опроса каким-то студентам уже исполнилось 22, а каким-то еще только 21. Если предположить, что месяц рождения опрашиваемых — это случайная величина, то в каком месяце, выбранном для опроса, больше вариативность возрастов? Например, стандартное отклонение? Например, если мы опрашиваем в январе (для удобства 31'го числа), то только январские скажут, что им уже 22, а остальным будет еще 21. По результатам такого опроса на одного 22'летнего будет приходиться в среднем одиннадцать 21-летних. Большинство значений возраста будут одинаковы и, следовательно, вариативность будет далека от максимума.

Предположим, все студенты 2000 г.р., опрос проводится в разные месяца 2020 года.

Даты и время рождения у всех разные. На 00:00 01.01.2020 — легко угадать, что всем студентам по 19 лет. Соответственно, корреляция стремится к 1.

Дальше каждый месяц корреляция снижается, т.к. у некоторых студентов наступают ДР и им исполняется 20 лет.

К 30.06-01.07.2020 проходит пол года, и за этот период у многих студентов уже наступил ДР, а у многих он еще предстоит. Соответственно, много людей кому еще 19 и много кому уже 20, большой разброс, угадать кому 19, а кому 20 трудно, если вообще реально. Соответственно, корреляция стремится к 0.

К концу года, 31.12.2020 в 23:59 ДР уже наступил у всех студентов, легко угадать, что всем по 20 лет. Соответственно, корреляция снова = 1.

Т.е. корреляция идет буквой U снижаясь от начала года до его середины и возрастая от середины года до его завершения.

№ нас есть вводные данные: **год рождения** и **возраст** студентов на **момент проведения опроса**. Также в задании сказано, что опрос проводился каждый **месяц**, что позволило заметить разницу в уровне корреляции. Это значит, что **год рождения** остается неизменным, а **возраст** и **дата опроса** — переменные.

Соответственно мы проводим опрос каждый месяц (всего 12 раз), и заносим все данные в таблицу **возраст** студента в **месяц опроса**, таким образом получаем данные для обработки по формуле КК и в итоге сравнить их между собой.

После проведения 12 опросов мы имеем таблицу с данными: **год рождения** студента, **возраст** студента на **каждый месяц проведения** опроса.

Дальше по **формуле КК**, которая давалась в этом разделе курса, получаем значение корреляции **год рождения** и **возраста** студентов  для каждого из 12 месяцев для дальнейшего сравнения.

Находим два месяца с самыми низкими значениями — это и будет ответом.

<details>
<summary><em>Код</em></summary>
...
Нужно просто догадаться в какие месяцы зависимость между годом рождения и возрастом будет наиболее слабая, т.е. наименее очевидная. А тут не надо считать. Просто представь, что ты спрашиваешь человека в каком году он родился. В каком случае ты с наибольшей вероятностью угадаешь его возраст — если сейчас стоит январь, июль или декабрь?

Последний месяц года и первый месяц года — корреляция будет наоборот максимальной, то есть мы можем угадать более менее точно. А вот в середине года у нас 50 на 50, то есть корреляция слабая. Середина года и есть два месяца. Было бы 13 месяцев — тогда правильным был бы 6-ой по счету месяц.

Ключевая ловушка: зависимость не от месяца, в котором кто родился, а от месяца, в котором проводилось анкетирование. Кто до сих пор не сталкивался, гляньте, что такое энтропия применительно к первичной оценке набора данных. Если и после этого будет непонятно, посмотрите картинки с деревьями, где указана энтропия для разных ситуаций.

Всё это прекрасно, но мы тут предполагаем, что распределение месяцев рождения между студентами равномерно. На практике самые популярные в мире месяцы рождения это август и сентябрь. Но и, конечно же, летние месяцы не являются совсем уж непопулярными, как и май. В общем, я бы указал, что мы считаем, что мы считаем распределение месяцев рождения между студентами равномерным. Будет проще если представить что 12 студентов родились в один и тот же год, но все в разные месяцы. Тогда сами подумайте какая корреляция 1-го января а какая 31 декабря.
...
### Глоссарий
Коэффициент ранговой корреляции Гудмена-Краскела (*Goodman-Kruskal's gamma*, $γ$)
: мера ассоциации между двумя порядковыми признаками, основанная на сравнении количества конкордантных (согласованных) и дискородантных (несогласованных) пар наблюдений.

Меры ассоциации
: статистические коэффициенты, показывающие силу и направление связи между категориальными переменными на основе таблиц сопряжённости.

Таблица сопряжённости (*contingency table*)
: способ представления совместного распределения двух категориальных переменных в виде двумерной таблицы для анализа их статистической связи. Строки таблицы соответствуют значениям одной переменной, столбцы — другой (количественные данные предварительно группируются в интервалы), а в ячейках указываются частоты совместного появления значений. Они универсальны для номинальных и порядковых шкал, позволяя выявлять зависимости между признаками.

Хи-квадрат статистика ($χ²$)
: непараметрический статистический критерий, используемый для проверки гипотез о соответствии наблюдаемых данных ожидаемым распределениям или о независимости категориальных переменных.
...
[^statistika-tabliczy-sopryazhennosti]: [Статистика. Таблицы сопряженности](https://nitforyou.com/statistika-tabliczy-sopryazhennosti/)
