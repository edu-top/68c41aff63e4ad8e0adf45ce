...
- <dfn title="количественный признак">Количественный</dfn> (<dfn title="числовой признак">числовой</dfn>): признак, выражаемый по своему физическому смыслу в виде некоторого вещественного (действительного) числа. Область его значений[^d-r] — вещественные числа, и сам признак имеет числовую природу. Есть числовые признаки, а есть числа, но числовыми признаками, как ни странно, не являющимися. Отличие первых от вторых в том, что над первыми можно производить операции свойственным числам, а над вторыми нет, ибо смысла они не имеют.

- <dfn title="порядковый признак">Порядковый</dfn>: не имеет числовой природы, он всего лишь задает порядок[^order] на некотором множестве объектов. Порядковые признаки иначе называют псевдоколичественными. Потому что имеют представление в виде числа, но операций математических не поддерживают кроме операций проверки на равенство (равно / не равно) и сравнения между собой (больше / меньше и т.д.). Также псевдоколичественные признаки позволяют работать с модой и медианой, но никак не со средними!
...
[^d-r]: Здесь немного матана. Область определения, область значений.
[^order]: А тут дискретная математика: Отношение порядка
...
!!! tip Задание
    ...
    Сформируйте таблицу с выборкой, содержащей признаки рисунков по вариантам из файла [features](https://docs.google.com/spreadsheets/d/1rCqfOmBJTDWbY5O71-SxzxSr_UZordQ85P5AavU4VL8/edit?usp=sharing).
