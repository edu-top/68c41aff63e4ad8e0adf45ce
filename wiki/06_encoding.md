...
Во-первых, необходимо проанализировать данные и определить, к какому типу они относятся. Посещаемость представлена в виде цветовой маркировки с разбивкой по дням, аттестационные отметки — в виде текстовых данных. В таком виде данные непригодны для обработки статистическими алгоритмами.

Как уже говорилось выше, коэффициент корреляции является двумерной описательной статистикой, которая применяется для описания двух простых переменных. Мы в таблице имеем посещаемость как многомерную характеристику каждого из изучаемых объектов (студентов). Для уменьшения размерности можно было бы объединить
