...
# Подготовка данных: агрегация и кодирование признаков
...
- [Кодирование данных](#кодирование-данных)
  - [Основные методы кодирования](#основные-методы-кодирования)
    ...
    - [Прямое кодирование для категориальных признаков](#прямое-кодирование-для-категориальных-признаков)
...
- [Практика](#практика)
  ...
  - [Практическая работа. Обработка и анализ признаков для неметрических шкал](#практическая-работа-обработка-и-анализ-признаков-для-неметрических-шкал)
    ...
    - [Примеры работ](#примеры-работ)
      - [Анализ корреляции температуры воздуха и посещаемости студентов](#анализ-корреляции-температуры-воздуха-и-посещаемости-студентов)
      - [Анализ корреляции между годом рождения и возрастом студента](#анализ-корреляции-между-годом-рождения-и-возрастом-студента)
      - [Анализ корреляции между годом рождения, возрастом студента и месяцем опроса](#анализ-корреляции-между-годом-рождения-возрастом-студента-и-месяцем-опроса)
- [Глоссарий](#глоссарий)
...
![Data science lifecycle](../img/fkg06ypnlajvkregmnrdawztwpi.png)
...
Например, если имеются ежечасные данные температуры и влажности, их можно агрегировать до среднесуточных значений, что сократит размерность и упростит анализ.
...
Таким образом, процедура приведения исходных данных (например, пол — м/ж или уровень облачности) в числовой вид для анализа называется <dfn title="кодированиее категориальных признаков">кодированием категориальных признаков</dfn> или <dfn title="преобразование качественных переменных">преобразованием качественных переменных</dfn>. В зависимости от типа данных и задачи используются разные методы:
...
<dfn title="порядковое кодирование">Порядковое кодирование</dfn> (*ordinal encoding*) — это метод преобразования категориального признака, для которого существует естественный порядок, в числовой вид, сохраняя этот порядок. Каждой категории присваивается целое число, отражающее её позицию в этом порядке.

В **порядковом кодировании** категория заменяется её номером. Этот метод хорошо подходит для признаков с порядком значений, таких, например, как уровень образования. Каждой категории присваивается числовой ранг согласно их естественному порядку (например, уровень облачности: ясно = 0, малая = 1, средняя = 2, высокая = 3, пасмурно = 4). Это сохраняет информацию о порядке, что важно для корреляционного анализа, учитывающего ранги. Так, в отличие от унитарного кодирования (*one-hot encoding*), которое создаёт отдельные бинарные признаки для каждой категории и не учитывает порядок, ординальное кодирование компактно и отражает информационный смысл порядка категорий.
...
- Широко используется в задачах машинного обучения для обработки номинальных (без внутреннего порядка) категориальных признаков.
...
**Недостатки циклического кодирования**:

- увеличивает размерность данных: вместо одного признака появляется пара $(x_1,
x_2)$, что может привести к "проклятию размерности" при большом числе цикличных категорий.

- требует знания периода $N$: метод работает только если точно известно общее количество категорий и их циклический порядок (например, для месяцев это очевидно, но для произвольных циклов — не всегда).

- предполагает равномерность интервалов: все категории должны быть равномерно распределены по циклу (месяцы по 30–31 день — приемлемо, но для неравномерных интервалов, как фазы Луны, точность падает).

- менее интерпретируемо: значения синуса и косинуса не дают прямого понимания категории (в отличие от one-hot), что усложняет анализ и отладку модели.

- не подходит для нецикличных категорий: применение к данным без цикличности (например, брендам товаров) искажает расстояния и ухудшает модель.
...
## Вывод
Агрегация и кодирование признаков играют ключевую роль в подготовке данных для машинного обучения, преобразовывая сырые, разнородные данные в структурированный числовой формат, который модели могут эффективно использовать для обучения и предсказаний. Агрегация (средние, суммы, минимумы по группам) позволяет сжимать информацию из множества наблюдений в компактные статистики, снижая шум и выделяя паттерны, в то время как кодирование (прямое, порядковое, частотное, средним, метками, унитарное, циклическое) устраняет категориальность, сохраняя семантику и расстояния между значениями. Вместе они минимизируют потерю информации, борются с проблемами размерности и повышают интерпретируемость, но требуют учета специфики данных — например, цикличности для временных признаков или редкости категорий для предотвращения утечек. В итоге качественная подготовка на этих этапах часто определяет успех модели, превращая хаос данных в предсказательную мощь.
...
#### Задание
Исследовать меру линейной связи между количественным и порядковым признаками с помощью ранговой корреляции в соответствии с данными индивидуального варианта, приводимыми в прилагаемом файле.

1. Выполнить предварительное кодирование порядковых признаков, выбрав наиболее подходящий метод. Обосновать выбор используемого метода.
2. Рассчитать коэффициент корреляции Спирмена $r_s$. Расчет всех промежуточных величин (ранги, ранговые разности, квадраты ранговых разностей и т.п.) привести в отчете.
3. Проверить статистическую значимость коэффициента корреляции с помощью t-статистики для уровня значимости α=0.05.*
4. Сделать выводы о силе и характере (направлении), а также статистической значимости связи.
...
Необходимо выбрать наиболее подходящий коэффициент корреляции (в зависимости от типа данных — бинарные или порядковые).
...
##### Анализ корреляции температуры воздуха и посещаемости студентов
...
##### Анализ корреляции между годом рождения и возрастом студента
...
##### Анализ корреляции между годом рождения, возрастом студента и месяцем опроса
...
## Глоссарий
Агрегация данных
: процесс объединения и обобщения исходных данных для получения новых признаков или сводной информации, которая влияет на качество модели в машинном обучении. Основная цель агрегации — упростить и структурировать данные, выявить тренды и закономерности, снизить шум и повысить информативность признаков. В контексте визуализации данных агрегация относится к процессу сбора, обобщения, консолидации и представления отдельных элементов данных в более полном и значимом формате, что облегчает анализ и интерпретацию информации.

Бинарные признаки (*binary features*)
: признаки, принимающие всего два значения, такие как семейное положение человека или индикатор, были ли у человека просрочки по кредитам.

Дихотомизация (*dichotomous coding*)
: процесс преобразования непрерывной или категориальной переменной в бинарную, то есть в две категории (обычно 0 и 1). В статистике и машинном обучении дихотомизация позволяет представить сложные или многозначные данные в упрощённом виде для дальнейшего анализа или построения моделей. Суть метода заключается в том, что при двух категориях (например, пол: м/ж) каждой присваивается 0 или 1, что удобно для большинства алгоритмов.

Категориальные признаки (*categorical features*)
: признаки, принимающие одно из $K$ дискретных значений, такие как профессия человека или город, в котором он родился. Такие признаки очень часто встречаются на практике. Другими словами, категориальные признаки — это признаки, которые описываются метками или строковыми значениями, такими как цвет, тип или категория объекта.

Кодирование категориальных данных
: процесс преобразования качественных признаков в числовую форму для дальнейшего анализа или использования в алгоритмах машинного обучения, которые обычно требуют числовых входов. Таким образом, процедура приведения исходных данных (например, пол — м/ж или уровень облачности) в числовой вид для анализа называется кодированием категориальных признаков или преобразованием качественных переменных.

Кодирование метки (прямое кодирование) (*label encoding*)
: простое преобразование каждого значения категориальных признаков в числовые значения. При этом каждой уникальной категории присваивается число, т.е. каждое значение категориального признака заменяется на соответствующее целое число.

Кодирование средним (*mean encoding*) или таргет-кодирование (*target encoding*)
: метод кодирования категориальных признаков, при котором каждой категории присваивается числовое значение, равное среднему значению целевой переменной для этой категории. Кодирование средним — компактный и максимально информативный способ кодирования категориального признака путём замены каждой категории на среднее прогнозируемого отклика _при условии этой категории_.

Порядковое кодирование (*ordinal encoding*)
: метод преобразования категориального признака, для которого существует естественный порядок, в числовой вид, сохраняя этот порядок. Каждой категории присваивается целое число, отражающее её позицию в этом порядке.

Частотное кодирование (*frequency encoding*)
: метод преобразования категориальных переменных в числовые значения, при котором каждой категории присваивается число, равное частоте её встречаемости в наборе данных. То есть категорию заменяют на то, насколько часто она встречается среди всех наблюдений.

Унитарное (однократное) кодирование (*one-hot encoding*, *dummy encoding*)
: метод преобразования категориального признака в числовой формат, при котором каждая уникальная категория превращается в отдельный бинарный признак (столбец). В полученной матрице для каждой записи один из столбцов принимает значение 1 (указывая на принадлежность к данной категории), а все остальные — 0.

Циклическое кодирование (*cyclic encoding*)
: метод преобразования категориальных признаков, которые имеют естественный циклический порядок, в числовые признаки с сохранением такой цикличности. Часто используется для признаков, представляющих циклы времени или циклические явления, например, часы суток, дни недели, месяцы года.

Эмбеддинг (*embedding*)
: представление определённой дискретной сущности (например, категории в категориальном признаке) вектором вещественных чисел.
