...
- [04. Характеристики признаков](#04-характеристики-признаков)
  ...
  - [Введение в математическую статистику](#введение-в-математическую-статистику)
...
- Научиться считать среднее значение, отклонение и медиану признаков.
- Выяснить, что среднее и медиана — не одно и то же.

### Результаты обучения
Изучение модуля по математической статистике позволяет студентам сформировать ключевые знания и компетенции для профессиональной деятельности, связанные с анализом и обработкой статистических данных. В результате освоения модуля студенты:

- Приобретают понимание основных понятий теории вероятностей и математической статистики.

- Осваивают методы сбора, обработки, группировки и визуализации данных.

- Учатся вычислять и интерпретировать числовые характеристики (средние значения, дисперсию, коэффициенты вариации и др.).

- Развивают навыки построения вариационных и эмпирических рядов, оценки формы распределения и однородности выборок.

- Овладевают способностью применять статистические методы при анализе прикладных задач в различных областях науки и техники.

- Формируют умение использовать программные средства для статистической обработки данных.

Таким образом, модуль обеспечивает системное владение теоретическими и практическими аспектами статистического анализа, что критично для научных исследований и решения профессиональных задач с использованием информации и данных.
...
-- | -- | --
Случайный эксперимент | Статистическое наблюдение (исследование) | Возможный результат одного наблюдения или эксперимента
...
Вероятность события | Относительная частота варианты (частотность) | Оценка вероятности на основе доли повторений события 
...
_Пример_. Для исследования жителей г. Ярославля (*генеральная совокупность*) на доминирующий цвет волос (*качественный признак*) рассмотрели 500 человек из разных районов города (*выборочная совокупность*). Получили следующие результаты:
...
Разность между верхней и нижней границами интервала называется <dfn title="интервальная разность">интервальной разностью</dfn> или <dfn title="длина частотного интервала">длиной частотного интервала</dfn>. В общем виде интервальную разность $k_i$ представим как
...
| $x_i$ | $z_i$ | $x_i$ | $z_i$ | $x_i$ | $z_i$ | $x_i$ | $z_i$ |
-- | -- | -- | -- | -- | -- | -- | --
0.30 | 589 | 0.37 | 602 | 0.29 | 537 | 0.34 | 596
0.33 | 614 | 0.33 | 544 | 0.34 | 574 | 0.36 | 605
0.37 | 612 | 0.34 | 545 | 0.39 | 570 | 0.33 | 575
0.36 | 572 | 0.33 | 562 | 0.37 | 540 | 0.34 | 570
0.31 | 548 | 0.30 | 576 | 0.38 | 575 | 0.36 | 550
0.35 | 535 | 0.29 | 572 | 0.32 | 562 | 0.38 | 557
0.32 | 593 | 0.30 | 555 | 0.38 | 601 | 0.37 | 558
0.39 | 582 | 0.33 | 555 | 0.37 | 587 | 0.34 | 587
0.30 | 538 | 0.32 | 518 | 0.38 | 587 | 0.35 | 580
0.32 | 566 | 0.32 | 539 | 0.33 | 614 | 0.36 | 560
...
<dfn title="гистограмма частот">Гистограммой частот</dfn> называют ступенчатую фигуру, состоющую из прямоугольников, основаниями которых служат частичные интервалы длиною $h$, а высоты равны отношению $\frac{n_i}{h}$ (<dfn title="плотность частоты">плотность частоты</dfn>).
...
#### Размах вариации
<dfn title="размах значений признака">Размах значений признака</dfn> (<dfn title="размах вариации">размах вариации</dfn>, *range*) $R(x)$ — разница между максимальным и минимальным значениями:
...
Если данные сгруппированы, то есть у нас есть значения признаков $x_i$ и частоты $n_i$ для каждого значения, то выборочное среднее считают так:
...
Для несгруппированных данных, когда есть просто набор наблюдений $x_1,x_2, \dots, x_n$, формула упрощается до:

$$ \bar{x} = \frac{\sum_{i=1}^n{x_i}}{n} $$

То есть оба варианта корректны, просто зависят от того, в каком виде у нас данные: напрямую или сгруппированные по частотам.
...
!!! todo "Задача на медиану и среднее"
...
#### Мода
<dfn title="мода">Мода</dfn> (*mode*) $m_o$ — это значение признака, которое встречается в выборке наиболее часто, то есть имеющее наибольшую частоту (число повторений). В дискретном вариационном ряду мода — это вариант с максимальной частотой. Таким образом, <dfn title="мода">мода</dfn> — значение, которое наблюдается в выборке чаще всего. Например, модой здесь $<2,0,1,1,3,2,3,2>$ будет 2.

А как же быть, если одинаковое количество разных значений? Это значит, что в выборке мода не одна или ее вовсе нет? Вот в конкретно данном примере $<-2, -1, 0, 1, 2>$ что является модой?
...
- если две соседних варианты имеют одинаковую наибольшую частоту, мода — среднее арифметическое этих вариант;

- если есть несколько вариант с одинаковой максимальной частотой, ряд называют <dfn title="бимодальный вариационный ряд">бимодальным</dfn> (2 моды) или <dfn title="полимодальный вариационный ряд">полимодальным</dfn> (более 2);

- вариационный ряд, имеющий единственную варианту с максимальной частотой, называют <dfn title="унимодальный вариационный ряд">унимодальным</dfn>.
...
> **Мода (в отличие от среднего и от медианы) имеет смысл и для номинальных (категориальных) признаков.**
...
где $n = \sum{n_i}$ — общее количество наблюдений (объем выборки).

Выборочная дисперсия показывает степень отклонения, но не позволяет простым образом соотнести это отклонение с самими значениями (из-за того, что измеряется в квадратах единиц значений). Каким образом еще можно охарактеризовать данное отклонение? Сделать это можно с помощью среднего квадратического отклонения.

#### Выборочное среднее квадратическое отклонение
<dfn title="среднее квадратическое отклонение выборки">Среднее квадратическое отклонение выборки</dfn> (<dfn title="выборочное стандартное отклонение">выборочное стандартное отклонение</dfn>, *standard deviation*) $S$ показывает среднее отклонение значений выборки от среднего. Является квадратным корнем из дисперсии

$$ S_P = \sqrt{S^2} = \sqrt{\frac{1}{n-1} \sum_{i=1}^n(p_i - \bar{p})^2} $$

!!! attention "Важное уточнение"

    Для расчета среднеквадратического отклонения в выборке сумма квадратов разностей делится на $n-1$, а для расчета среднеквадратического отклонения в генеральной совокупности — на $n$. Смысл в том, что формула с $\frac{1}{n-1}$ в среднем дает более лучшее приближение к истинному отклонению (которое мы не знаем). Это т.н. несмещенное среднеквадратичное отклонение. Если вкратце, считается, что мы оцениваем не все значения параметра, а выборку из этих значений, некие наблюдения. Данная формула $(n-1)$ дает лучшую оценку среднеквадратичного отклонения для выборки, чем просто $n$. Дело в том, что у выборки величины отклонения всегда получаются меньше, и чтобы это компенсировать, делят на $n-1$.[^несмещенная-оценка]
...
Основные числовые показатели дают всестороннюю характеристику выборки: **центральную тенденцию** (мода, медиана, среднее), **разброс** (размах, дисперсия, стандартное отклонение), и **степень вариативности** относительно масштаба среднего (коэффициент вариации).

Эти показатели позволяют не только описать данные, но и выявить их ключевые свойства, влияющие на выбор методов анализа. На основе этих значений можно делать выводы о форме распределения, однородности, наличии выбросов и особенностях данных, что важно для корректного статистического анализа и принятия решений.

Сравнивая меры (например, среднее > медиана — правосторонняя асимметрия), можно предположить нормальность (все меры близки), наличие выбросов (размах велик при малой дисперсии) или однородность (низкий CV). Это определяет тесты (t-критерий для нормальных данных, Манн-Уитни для ненормальных) и помогает избежать ошибок в моделировании.
...
- **Мода** (*mode*, $m_o$) — наиболее часто встречающееся значение в наборе. Полезна для категориальных данных или для выявления повторяющихся наиболее типичных значений.
...
#### Цель работы
Практическое освоение анализа статистических данных.

#### Задание
Выполнить набор представленных задач в соответствии с индивидуальным вариантом.

1. Составить дискретный и интервальный вариационные ряды для выборки данных по исследуемому признаку $X$.
2. Построить гистограмму распределения значений исследуемого признака.
3. Определить основные числовые характеристики исследуемого признака:
     - объем выборки $n$;
...
*Набор исходных данных (выборочная совокупность)*
...
$x_i$ | 160 | 163 | 164 | 165 | 166 | 167 | 168 | 169 | 170 | 171 | 172 | 173 | 174 | 175 | 176 | 177 | 178 | 179 | 180 | 182
-- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | --
$n_i$ | 1 | 2 | 2 | 1 | 1 | 1 | 2 | 2 | 1 | 2 | 1 | 1 | 1 | 2 | 1 | 3 | 1 | 2 | 2 |1
...
Унимодальный вариационный ряд
: вариационный ряд, имеющий единственную варианту с максимальной частотой.
