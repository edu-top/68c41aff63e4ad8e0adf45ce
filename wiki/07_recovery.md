...
Выбор методов импутации для порядковых данных отличается от количественных, несмотря на числовое представление: порядковые (оценки 0-5, уровень "низкий/средний/высокий") имеют дискретный порядок без равных интервалов, поэтому среднее может дать нецелые значения (2.7), искажая шкалу.
