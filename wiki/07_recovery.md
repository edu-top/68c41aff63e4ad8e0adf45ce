...
- **Интерполяционный многочлен Лагранжа** строит полином степени $n$ для $n+1$ точек через базисные множители:
    ...
    Метод подходит для неравноотстоящих узлов, но вычислительно дорог при большом $n$ из-за $O(n^2)$.

- **Интерполяционные формулы Ньютона** использует разделённые разности для рекуррентного построения:

    $$ P_n(x) = f[x_0] + f[x_0,x_1](x-x_0)+\dots+f[x_0,\dots,x_n] \prod_{i=0}^{n-1}{(x-x_i)} $$

    Для равноотстоящих узлов упрощается до формы с конечными разностями $Δ^k f(x_0)$, эффективно добавляя точки. Преимущество — удобство для последовательной интерполяции.
...
- **Метод конечных разностей** заменяет производные разностными соотношениями:
    ...
    Идеален для равноотстоящих данных, компактен в таблицах разностей.
